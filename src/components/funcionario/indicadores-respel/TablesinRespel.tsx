...
import { getEstablecimientosRespel, type EstablecimientosRespelResponse } from '@/services/funcionario/raee-respel-rcd/getEstablecimientosRespel.service';
...
export default function EstablecimientosTabla({ periodo }: { periodo?: string }) {
  ...
  const [data, setData] = useState<EstablecimientosRespelResponse | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await getEstablecimientosRespel({
        ...
        setError('Error al cargar establecimientos RESPEL');
  ...
  // Reset página al cambiar periodo
  useEffect(() => {
    setPage(1);
  }, [periodo]);
  ...
  return (
    <div className=' w-full space-y-4 mt-6'>
      <div className='sticky top-0 bg-white z-10'>
        <h2 className='text-lg font-semibold text-gray-800 mb-1'>RESPEL sin recolección autorizada (CVC/DAGMA)</h2>
        <p className='text-sm text-gray-500 mb-4'>Mapa coroplético de RESPEL sin recolección autorizada (CVC/DAGMA)r</p>

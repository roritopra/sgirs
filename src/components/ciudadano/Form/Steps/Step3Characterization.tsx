...
import { indexById, groupOpcionesByPregunta, toDynamicQuestions } from "@/services/ciudadano/FormService/questions.adapter";
import { CitizenFormSkeleton } from "@/components/skeletons/CitizenFormSkeleton";

export const Step3Characterization = () => {
  ...
  const initialValues = useMemo(() => {
    // Sincroniza con FormState existente para mantener compatibilidad
    return questions && questions.length > 0
      ? {
          [questions[0].id]: state.caracterizacionResiduos,
        }
      : {};
  }, [questions, state.caracterizacionResiduos]);

  const attachments = useMemo(() => {
    return questions && questions.length > 0
      ? {
          [questions[0].id]: state.dynamicAttachments?.[questions[0].id],
        }
      : {};
  ...
  const handleValueChange = (id: string, value: any) => {
    ...
    // También reflejar en el estado tradicional para validación/resumen actuales
    updateForm({ caracterizacionResiduos: value as boolean | null });
  };

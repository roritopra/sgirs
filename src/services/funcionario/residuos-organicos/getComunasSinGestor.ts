...
export async function getComunasSinGestor(useMock: boolean): Promise<Comuna[]> {
	if (useMock) {
		return [
			...
			{ id: 7, comuna: 7, cantidad: 10, porcentaje: 5.0 },
	...
	const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/rso/sin-gestor`);
	if (!res.ok) throw new Error('Error al cargar comunas sin gestor');
	return res.json();
}

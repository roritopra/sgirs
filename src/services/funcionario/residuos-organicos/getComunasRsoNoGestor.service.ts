...
// GET /api/v1/dashboard/rso/establecimientos-sin-gestion-porcentaje
// Respuesta esperada: [{ posicion, comuna: string, cantidad, porcentaje }]
export async function getComunasRsoNoGestor(options: { periodo?: string } = {}): Promise<Comuna[]> {
    const { periodo } = options;
    const base = '/api/v1/dashboard/rso/establecimientos-sin-gestion-porcentaje';

...
export type TipoEstablecimientos = 'con_compostaje' | 'sin_gestion';

function endpointFor(tipo: TipoEstablecimientos): string {
  if (tipo === 'con_compostaje') return '/api/v1/dashboard/rso/establecimientos-con-compostaje';
  return '/api/v1/dashboard/rso/establecimientos-sin-gestion';
}
...
export async function getEstablecimientosRsoTabla(params: {
  tipo: TipoEstablecimientos;
  search?: string;
  page?: number;
  limit?: number;
  periodo?: string;
}): Promise<EstablecimientosTablaResponse> {
  const { tipo, search, page = 1, limit = 20, periodo } = params;
  ...
  const url = `${endpointFor(tipo)}?${qs.toString()}`;

...
export interface EstablecimientoRcd {
  id?: string;
  nombre: string;
  responsable: string;
  direccion: string;
...
export interface EstablecimientosRcdResponse {
  data: EstablecimientoRcd[];
  total: number;
  page: number;
  limit: number;
  totalPages: number;
...
export interface GetEstablecimientosRcdParams {
  search?: string;
  estado?: 'gestor_aut' | 'gestor_no_aut';
  page?: number;
  limit?: number;
...
const normalize = (item: any, params?: GetEstablecimientosRcdParams): EstablecimientoRcd => ({
  id: String(item?.id ?? ''),
  nombre: String(
    item?.nombre ??
...
export async function getEstablecimientosRcd(
  params?: GetEstablecimientosRcdParams
): Promise<EstablecimientosRcdResponse> {
  ...
  const url = `/api/v1/dashboard/RCD/recolecciones/establecimientos${sp.toString() ? `?${sp.toString()}` : ''}`;
